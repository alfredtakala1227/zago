angular.module('app').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('index.html',
    "<!DOCTYPE html><html lang=en xml:lang=en ng-app=app><head><meta charset=\"UTF-8\"><meta name=google content=notranslate><meta http-equiv=Content-Language content=en_US><base href=\"/\"><meta name=viewport content=\"width=device-width,height=device-height,initial-scale=1\"><title>Zago | Under Construction</title><link rel=icon type=image/png href=images/favicon.ico><link rel=stylesheet href=\"css/style.css\"><script src=js/lib/polyfills.min.js></script><script src=js/lib/modernizr.js></script><script src=js/lib/mobile-detect.min.js></script><script src=js/lib/mobile-detect-modernizr.js></script><script src=js/lib/ios-orientationchange-fix.js></script><script src=js/lib/overthrow.min.js></script><script src=js/lib/jquery-1.8.3.min.js></script><script src=js/lib/angular.min.js></script><script src=js/lib/angular-ui-router.min.js></script><script src=js/lib/angular-sanitize.min.js></script><script src=js/lib/angular-scroll.min.js></script><script src=js/app/enhance.js></script><script src=js/app/modernizr-script.js></script><script src=js/app/main.js></script><script src=js/app/templates.js></script></head><body id=body ng-controller=AppCtrl><div id=container><img id=siteID class=siteID alt=\"Zago LLC Logo\" src=images/zago_logo.png ng-click=\"route('home', true)\"><nav id=mainNav class=overthrow><div id=menuWrapper><div id=menu><ul><li><a ui-sref=home ng-class=\"{active: currentState == 'home'}\" title=Home>Zago</a></li><li><a ui-sref=projects ng-class=\"{active: currentState == 'projects'}\" title=\"Zago Projects\">Projects</a></li><li><a ui-sref=team ng-class=\"{active: currentState == 'team'}\" title=\"Meet Z Team\">Team</a></li><li><a href=\"http://zagolovesyou.tumblr.com/\" ng-click=toggleMenu(true) target=_blank title=\"Blog Love\">Love</a></li></ul></div><div class=menuContacts office-list></div></div></nav><div class=buttonWrapper><p id=menuPrompt>Click for<br>Menu</p><button id=menuButton type=button ng-click=toggleMenu()></button></div><button id=scrollTop type=button ng-click=scrollTop()></button><div id=pageContent class={{pageView}} ui-view autoscroll=true></div><footer id=footer ng-class=\"{homePage: pageView == 'homePage', lovePage: pageView == 'lovePage'}\"><section class=collaboration><p>Let's work together.</p><p>Come have coffee & say ciao.</p><a href=mailto:info@zagollc.com>info@zagollc.com</a></section><section class=intern><p class=footerCiaoline>Say ciao at <a href=mailto:info@zagollc.com>info@zagollc.com</a></p><p>We take interns in NYC for</p><p>3 to 6 month engagements.</p><p>Inquire at <a href=mailto:intern@zagollc.com>intern@zagollc.com</a></p></section><div class=officeListing office-list></div><section class=social social-buttons></section><div id=test-sprites><ul><li class=downArrow></li><li class=downArrow-white></li><li class=upArrow></li><li class=upArrow-white></li></ul></div></footer></div><!--[if lt IE 9]><script type=\"text/javascript\" src=\"js/unsupported.js\"></script><![endif]--></body></html>"
  );


  $templateCache.put('pieces/grid.html',
    "<div class=gridBox><div ng-repeat=\"section in sections\"><div ng-class=\"{ gridWrapper: checkModulo($index, 6) }\"><div ng-class=\"{ grid: checkModulo($index, 3), gridLeft: checkModulo($index, 6) }\"><div ng-if=\"gridType == 'project-section'\" project-section></div><div ng-if=\"gridType == 'team-section'\" team-section></div></div></div></div></div>"
  );
  ...
  $templateCache.put('pieces/project_nav.html',
    "<nav id=projectNav><ul><li><a ui-sref=projects><div class=\"navButton project-back\"></div><div class=navTitle>All Projects</div></a></li><li><a href=projects/{{prevProject.id}}><div class=\"navButton project-prev\"></div><div class=navTitle ng-bind-html=prevProject.content.name></div></a></li><li><a href=projects/{{nextProject.id}}><div class=\"navButton project-next\"></div><div class=navTitle ng-bind-html=nextProject.content.name></div></a></li></ul></nav>"
  );


  $templateCache.put('pieces/project_section.html',
    "<section data-id={{sections[$index].id}}><div class=imgWrapper><img ng-src={{sections[1].featured_image.url}}><div class=overlay></div></div><h2>{{sections[$index].title}}</h2><h3>{{sections[$index].client}}</h3></section>"
  );
